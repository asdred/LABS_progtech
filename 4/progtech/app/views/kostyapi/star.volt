{{ link_to("kostyapi/createstar/", 'Создать', "class": "btn btn-primary") }}
<table class="table table-bordered table-striped">
    <tr>
        <td class="table_header hidden">id</td>
        <td class="table_header">Название</td>
        <td class="table_header">Возраст</td>
        <td class="table_header">Всс</td>
        <td class="table_header">Система</td>
...
?>
    <tr>
        ...
        <td class="table_body"><?=$data->age?></td>
        <td class="table_body"><?=$data->weight?></td>
        <td class="table_body"><?=$data->solar_system?></td>
        <td class="table_body"><?=$data->type?></td>
        <td width="7%">{{ link_to("kostyapi/editstar/" ~ data.id, '<i class="glyphicon glyphicon-edit"></i> Изменить', "class": "btn btn-default") }}</td>
        <td width="7%">{{ link_to("kostyapi/deletestar/" ~ data.name, '<i class="glyphicon glyphicon-remove"></i> Удалить', "class": "btn btn-default") }}</td>
    </tr>	
<?php
    }
?>
        ...
        <td colspan="7" align="right">
            <div class="btn-group">
                <form action="/Kostyapi/select" method="post">
                    <input type="hidden" name="table" value="звезда">
