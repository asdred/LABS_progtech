...
{   
	...
	{
		$curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'https://laika-workspace-cloned-laika-boss.c9users.io/Api/'.$table."?page=".$page);
	...
	{
		$curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'https://laika-workspace-cloned-laika-boss.c9users.io/Api/id'.$table."?id=".$id);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER,true);
		return json_decode(curl_exec($curl));
	}
	...
	{
		$curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'https://laika-workspace-cloned-laika-boss.c9users.io/Api/name'.$table."?name=".$name);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER,true);
		return json_decode(curl_exec($curl));
	}
    ...
    { 
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'https://laika-workspace-cloned-laika-boss.c9users.io/Api/add'.$table); 
    ...
    { 
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'https://laika-workspace-cloned-laika-boss.c9users.io/Api/upd'.$table); 
        curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "POST"); 
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data, JSON_UNESCAPED_UNICODE)); 
        curl_setopt($curl, CURLOPT_RETURNTRANSFER,true); 
        return json_decode(curl_exec($curl)); 
    ...
    { 
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'https://laika-workspace-cloned-laika-boss.c9users.io/Api/del'.$table."?name=".$name); 
    ...
    public function editgalaxyAction($id) {
        ...
        if (!$this->request->getPost()) {
            ...
            $this->view->types = $this->getFullTable("typegalaxy");
            $this->view->clusters = $this->getFullTable("cluster");
    ...
    public function editplanetAction($id) {
        ...
        if (!$this->request->getPost()) {
            ...
            $this->view->solarsystems = $this->getFullTable("solarsystem");
            $this->view->types = $this->getFullTable("typeplanet");
    ...
    public function editstarAction($id) {
        ...
        if (!$this->request->getPost()) {
            ...
            $this->view->solarsystems = $this->getFullTable("solarsystem");
            $this->view->types = $this->getFullTable("typestar");
    ...
    public function editsolarsystemAction($id) {
        ...
        if (!$this->request->getPost()) {
            ...
            $this->view->galaxys = $this->getFullTable("galaxy");
    ...
    public function editblackholeAction($id) {
        ...
        if (!$this->request->getPost()) {
            ...
            $this->view->galaxys = $this->getFullTable("galaxy");
            $this->view->types = $this->getFullTable("typeblackhole");
    ...
    {   
        $this->view->data = $this->getFullTable("cluster");
        $this->view->pages = $pages;
    }
    
    private function getFullTable($table = "")
    {
        if($out = $this->curvl($table, 1)) {
            
            $data = array($out);
            $pages = $out->count_pages;
            
            if ($pages > 1) {
                
                for($i = 1; $i < $pages; $i++) {
                    
                    $next_out = $this->curvl($table, $i + 1);
                    
                    array_push($data, $next_out);
                }   
            }
