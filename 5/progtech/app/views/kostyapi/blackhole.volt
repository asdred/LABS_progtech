{{ link_to("kostyapi/createblackhole/", 'Создать', "class": "btn btn-primary") }}
...
?>
    <tr>
        ...
        <td width="7%">{{ link_to("kostyapi/editblackhole/" ~ data.id, '<i class="glyphicon glyphicon-edit"></i> Изменить', "class": "btn btn-default") }}</td>
        <td width="7%">{{ link_to("kostyapi/deleteblackhole/" ~ data.name, '<i class="glyphicon glyphicon-remove"></i> Удалить', "class": "btn btn-default") }}</td>
    </tr>	
<?php
    }
?>
        ...
        <td colspan="7" align="right">
            <div class="btn-group">
                <form action="/Kostyapi/select" method="post">
                    <input type="hidden" name="table" value="чёрная дыра">
